...
import 'package:spotify/bloc/player_bloc/player_state.dart';
import 'package:spotify/db_helper/db_helper.dart';


import 'package:spotify/bloc/player_bloc/player_event.dart';



class PlayerBloc extends Bloc<PlayerEvent, PlayerStateNew> {
  ...
  Future<void> _onPlayPauseEvent(
      PlayPauseEvent event, Emitter<PlayerStateNew> emit) async {
    if (isPlaying) {
      player.pause();
      emit(state.copyWith(
        isPlaying: false,
  ...
  Future<void> _onPlayEvent(
      OnPlayEvent event, Emitter<PlayerStateNew> emit) async {
  ...
  Future<void> _progressUpdate(
      ProgressUpdateEvent event, Emitter<PlayerStateNew> emit) async {
  ...
  Future<void> _onTapFavouriteEvent(
      OnTapFavouriteEvent event, Emitter<PlayerStateNew> emit) async {
    final alreadyExist =
  ...
  void _onTapForwardEvent(OnTapForwardEvent event, Emitter<PlayerStateNew> emit) {
  ...
  void _onTapBackwardEvent(
      OnTapBackwardEvent event, Emitter<PlayerStateNew> emit) {
